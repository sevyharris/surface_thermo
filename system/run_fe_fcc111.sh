#!/bin/bash
#SBATCH --job-name=run_gemnet_fe_fcc111
#SBATCH --mem=20Gb
       ...
       for adsorbate in "${adsorbates[@]}"; do
           for site in 'ontop' 'bridge' 'fcc' 'hcp'; do
               for rotation in 0 90 180; do
                   ...
                   python relax_system.py --metal "$metal" --crystal_structure "fcc" --facet "$facet" --adsorbate "$adsorbate" --site "$site" --rotate "$rotation"
